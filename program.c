...
// #define FIRST_PREMIUM 11
// #define FIRST_NONMEMBER 21
...
space *space_availableorNot(char *prev_membership)
{
    ...
    // Fallback: If reserved area is full, search from head (space 1 to 50)
    if (ans == NULL)
    {
        temp = parkingLot;
        while (temp && !flag)
...
void exitVehicle(char *vehicleNumber)
{
    ...
    // Calculate cost according to Payment Policy: 100 Rs flat for first 3 hours, 50 Rs for each extra hour
    int cost;
    if (duration_hours <= 3)
    {
        cost = 100;
    ...
    while (choice >= 1 && choice < 9)
    {
        ...
        case 1:
            ...
            if (prev_membership != NULL && empty_Space_for_vehicle != NULL && prev_membership->spaceID == 0)
            {
                ...
                // updateVehicle_DB(prev_membership,ownerName);
                prev_membership->spaceID = empty_Space_for_vehicle->id;
                // strcpy(prev_membership->OwnerName,ownerName);
                struct time *t = (struct time *)malloc(sizeof(struct time));
                take_arrival_time(t);
